...
gophercloudtest() {
  if [[ -n $1 ]] && [[ -n $2 ]]; then
    pushd  $GOPATH/src/github.com/samuelbernardolip/gophercloud
    go test -v -tags "fixtures acceptance" -run "$1" github.com/samuelbernardolip/gophercloud/acceptance/openstack/$2 | tee ~/gophercloud.log
    popd
fi
}
```
